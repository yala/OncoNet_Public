...
class RiskFactorVectorizer():
    def __init__(self, args):
        ...
        self.use_second_order_risk_factor_features = args.use_second_order_risk_factor_features
        if self.use_second_order_risk_factor_features:
            self.feature_names = self.get_2nd_order_feature_names(self.feature_names)
    ...
    def transform(self, patient_factors, exam_factors):
        ...
        return torch.cat(risk_factor_vecs)
    ...
    def get_risk_factors_for_sample(self, sample):
        ...
        if self.use_second_order_risk_factor_features:
            risk_factor_vector = self.convert_to_2nd_order(risk_factor_vector)
        return risk_factor_vector

    def convert_to_2nd_order(self, vector):
        second_order_vector = torch.zeros(int(binom(len(vector), 2)))
        k = 0
        for i in range(len(vector)):
            for j in range(i + 1, len(vector)):
                second_order_vector[k] = 1 if vector[i] == 1 and vector[j] == 1 else 0
                k += 1
        return torch.cat([vector, second_order_vector])

    def get_2nd_order_feature_names(self, feature_names):
        second_order_feature_names = [''] * int(binom(len(feature_names), 2))
        k = 0
        for i in range(len(feature_names)):
            for j in range(i + 1, len(feature_names)):
                second_order_feature_names[k] = '{}_and_{}'.format(feature_names[i], feature_names[j])
                k += 1
        return feature_names + second_order_feature_names
