...
def get_model_by_name(name, allow_wrap_model, args):
    '''
        ...
        - allow_wrap_model: whether or not override args.wrap_model and disable model_wrapping. can be useful to meta_models where we don't want to remove fcs
    ...
    return wrap_model(model, allow_wrap_model, args)

def wrap_model(model, allow_wrap_model, args):
    if args.multi_image:
    ...
    if args.num_gpus > 1 and args.data_parallel and not isinstance(wrapped_model, nn.DataParallel):
...
def get_output_size(model, shape, channels, cuda):
    ...
    input = ag.Variable(torch.rand(bs, channels, *shape))
    if cuda:
        input = input.cuda()
        model = model.cuda()
